...
#define  	CA_MAX_PRODUCT_NUM		(512)/*可购买产品包最大数量*/
...
/* 授权来源*/
#define 	CA_ENTITLE_CARD		(0)	/*来自智能卡授权*/
#define 	CA_ENTITLE_GROUP	(1)	/*来自集团用户授权*/ 
#define 	CA_ENTITLE_PARENT	(2)	/*来自主卡授权*/
...
typedef struct _T_CA_IPPVPrice{
	GYU8    m_byEventPriceType; 	/* 0x0-一次性扣费, IPPV节目; 0x1-单位时段扣费, IPPT节目*/
...
typedef struct _T_CA_IPPVInfo{
	...
	GYU16   m_wEventId;  			/* 当前观看节目的事件id, IPPT时使用 */
	GYU32	m_dwEvent_Ex_Sn;		/* IPPV节目的事件ID */
...
/*一个产品包的购买信息*/
typedef struct _T_CA_EntitleInfo
{
	GYU16 m_byProductID; 	/*产品包编号*/
	GYU8	m_bVideoRecordEnabled; /* 是否可录像*/
	GYU8 	m_byComeFrom; /* 0-智能卡授权, 1-集团授权, 2-主卡授权*/
	T_CA_Date  m_expiredDate; /*过期日期*/
}T_CA_EntitleInfo;

/* 所有产品授权信息 */
typedef struct _T_CAProductEntitleInfo{
	GYU16 m_wProductCount; /* 产品包数量 */
	T_CA_EntitleInfo m_entitleInfo[CA_MAX_PRODUCT_NUM]; /*每个产品包的购买详细信息*/
}T_CAProductEntitleInfo; 
