...
#include "win_signal_obj.h"

#define WIN_SH_IDX                  WSTL_WIN_2
#define LOCK_IDX                    WSTL_BAR_05//WSTL_YU_ORANGE
#define UNLOCK_IDX                  WSTL_BAR_05//WSTL_YU_ORANGE
#define PROGRESS_COLOR_BLUE         WSTL_BAR_05//WSTL_YU_ORANGE
#define PROGRESS_COLOR_GREEN        WSTL_BAR_05//WSTL_YU_ORANGE
...
BOOL win_signal_update()
{
	...
	if(lock)
	{
		sprintf(str, "%d.%d dB", cn_noise / 10, cn_noise % 10);
	}
	else
	{
		sprintf(str, "0 dB");
	...
	sprintf(str, "%d E-7", ber);
	txt = &sig_txt_BER_uint;
	OSD_SetTextFieldContent(txt, STRING_ANSI, (UINT32)str);
	...
	sprintf(str, "%d.%d dBuv", temp, dot);
	txt = &sig_txt_level_uint;
	OSD_SetTextFieldContent(txt, STRING_ANSI, (UINT32)str);
