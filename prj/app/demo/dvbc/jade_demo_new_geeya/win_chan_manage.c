...
#define TV_ID 1
#define RD_ID 2
#define POPUP_MSG_LEFT	375
#define POPUP_MSG_TOP 190
...
static PRESULT chan_manage_con_item_callback(POBJECT_HEAD pObj, VEVENT event, UINT32 param1, UINT32 param2)
{
	...
	switch (event)
	{
		case EVN_UNKNOWN_ACTION:
			...
			if(unact == VACT_ENTER)
			{
				chan_index_flag = 1;//标志从win_chan_manage进入win_chan_edit
				...
				if(id == TV_ID)
				{
					...
					sys_data_set_cur_chan_mode(TV_CHAN);// add code by xc.liu
				...
				if(NULL != windows_chan_manage_list[id - 1])
				{
					if(OSD_ObjOpen(windows_chan_manage_list[id - 1], 0xFFFFFFFF) != PROC_LEAVE)
						menu_stack_push(windows_chan_manage_list[id - 1]);
				}
			}
			break;
		default:
