 ...
 *
 * Filename	: iso_5937.c
 * Description	: ISO-5937 charset mapping file.
 * Generated by	: ISO-Convertor, powered by Delphi
 *
 * History:
 * 20060426	Zhengdao Li	0.1.002		Add support for Euro mark.
 */
#include <types.h>
 ...
 INT32 iso6937_convert_ch(UINT8 *iso6937, INT32 length, UINT16 *unicode, INT32 maxlen)
{
	...
	for(si=di=0; (si<length)&&(di<maxlen); si++) 
	{
		...
		if(src<0xA8)
		{
			if((src==0x8A)||(src=='\r')||(src=='\n'))
				unicode[di++] = U16_TO_UNICODE((UINT16)'\n');
			else if(src==0xA4)
				unicode[di++] = U16_TO_UNICODE(0x20AC); /* Euro currency mark € */
			else if(src==0xA0)
				unicode[di++] = U16_TO_UNICODE((UINT16)0x20); /* NBSP*/
		...
		{/* C0-CF table */
			...
			else if ((src!=0xC0)&&(src!=0xC9)&&(src!=0xCC))
			{
				...
				src = iso6937[++si];
				if ((value = u8u16_lookup(src, entry->table, entry->count))==0xFFFF) 
				{
					unicode[di++] = U16_TO_UNICODE(iso6937_map_C0_CF[iso6937[si-1]-0xC0]);
		...
		//"SHY" need to special process, UI will process it according to the display situation, so don't convert it to hyphen character
		else if(src==0xFF)
		{
...
INT32 iso6937_convert_ch(UINT8 *iso6937, INT32 length, UINT16 *unicode, INT32 maxlen)
{
	/*register */INT32 si, di;
	/*register */UINT16 src, value;
	for(si=di=0; (si<length)&&(di<maxlen); si++) {
		src = iso6937[si];
		...
		if ((src>=0x80)&&(src<=0x9F)) { /* control code, ignore them */
			...
			unicode[di++] = U16_TO_UNICODE(iso6937_map_D0_FF[src-0xD0]);
		} else if ((src!=0xC0)&&(src!=0xC9)&&(src!=0xCC)){ /* 0xC1-0xCF combined code */
			if (length-si<2)
				break;
			else {
				struct lookup_table *entry = &iso6937_lookup_table[src-0xC1];
