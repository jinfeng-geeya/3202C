...
void cas_demux_free(UINT8 free_type)	// 1, free ecm; 2, free emm
{
	...
	if(GY_ECM_REQUEST_ID == free_type)	// free req_id == 1
	{
		cas_demux_free_channel(1);
	}
	else if(GY_EMM_REQUEST_ID == free_type)	// free req_id >= 2
	{
		for(i=2; i<4; i++)	
...
static void conti_rcv_sec_cb(struct get_section_param * param)
{
	...
	//libc_printf("\n\nGYCA GYCA_TableReceived before tick: %d\n\n",osal_get_tick());
	//osal_task_sleep(100);
#ifdef RECV_NOT_CONTINUE
	...
	//libc_printf("\n\nGYCA GYCA_TableReceived after tick: %d\n\n",osal_get_tick());
#ifndef RECV_NOT_CONTINUE
	if (GY_ECM_REQUEST_ID == tmp_channle->req_id[0])//if ECM, release channel
	{
		for(i=0; i<MAX_SEC_MASK_LEN; i++)
		{
			tmp_channle->mask_value.value[param->sec_hit_num][i] = 0;
...
INT32 gyca_setcw(WORD wEcmPID, BYTE byKeyLen, const BYTE * szOddKey, const BYTE * szEvenKey, BOOL  bTapingControl)
{
	UINT8 i;
	UINT16 ret;
	UINT8 rating;
...
void gy_set_pdsd(UINT32 value)
{
	PDSD = value;
}

INT32	GYSTBCA_GetNitValue(DWORD * pdwData)
{
